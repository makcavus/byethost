...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValueExplicit('A'.$row, $ilkod.'.'.$ilcekod, PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValue('B'.$row, mb_convert_encoding ($ilceninadi, "UTF-8", "ISO-8859-9" ))
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('A'.$row, mb_convert_encoding ($topil, "UTF-8", "ISO-8859-9" ))
...
$objPHPExcel->setActiveSheetIndex(1)->getStyle("A$row:T$row")->applyFromArray($styleThinBlackBorderOutline);
   ...
   $row++;
   $row=$row+1;
   $durum=$yilgelen.'-'.$ilinadi.' '.$iltoplami;
...
header('Content-Disposition: attachment;filename="Degerlendirme Raporu Yıl.xls"');
