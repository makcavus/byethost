...
$atarih=mysqli_query($dbh, "SELECT date_format(v175, '%d.%m.%Y')  AS yeni_tarih FROM veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen')") ;
while($duztarih=mysqli_fetch_array($atarih)){
$dtarih=$duztarih['yeni_tarih'];
}
$btarih=mysqli_query($dbh, "SELECT date_format(v178, '%d.%m.%Y')  AS yeni_tarih FROM veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen')") ;
...
from veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen') group by vocadi order by vocadi desc") ;
$oncekiadnks = @mysqli_query($dbh, "select sum(v1) as v1,sum(v3) as v3 from veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vayadi='OCAK') group by vocadi order by vocadi asc") ;
}else{
$resultvyil = @mysqli_query($dbh, "select ilceidi as iii, vocadi as vvv,
sum(v1) as v1,
sum(v2) as v2, 
...
from veri where(ilidi='$ilgelen' and vyiladi='$yilgelen' and ilceidi='$ilcegelen') group by vocadi order by vocadi desc") ;
$oncekiadnks = @mysqli_query($dbh, "select sum(v1) as v1,sum(v3) as v3 from veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen' and vayadi='$aygelen') group by vocadi order by vocadi asc") ;
}
...
$ahkod=substr(trim($socadi),6,20);
$kodu=$ilkod.'.'.$ilcekod.'.'.$ahkod;
if(substr(trim($socadi),6,3)>9999999){
$ahkod=substr(trim($socadi),6,3);
}else if(substr(trim($socadi),6,3)>9 and substr(trim($socadi),6,3)<9999999){
$ahkod=substr(trim($socadi),7,2);
}else{
	if(strlen($socadi)==9){
$ahkod=substr(trim($socadi),8,3);
		}elseif(strlen($socadi)>9){
	$ahkod=$ahkod;	
	}
$socadi=$kodu;
}
	if($yilgelen==""){
		$nuf=@mysqli_query($dbh,"select distinct vyiladi,vayadi from veri where ilidi='$ilgelen' and ilceidi='$ilcegelen'");
		$yilgelen_yaz='TÜM YILLAR';
	}elseif($ilgelen>0 and $yilgelen>0){
		$nuf=@mysqli_query($dbh,"select distinct vyiladi,vayadi from veri where ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen'");
...
$objPHPExcel->getSheet(1)->setTitle($ahkod);
...
$toncekiadnks = @mysqli_query($dbh, "select sum(v1) as v1,sum(v3) as v3 from veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vayadi='OCAK')") ;
}else{
$tresultvyil = @mysqli_query($dbh, "select 
sum(v1) as v1,
sum(v2) as v2, 
...
$toncekiadnks = @mysqli_query($dbh, "select sum(v1) as v1,sum(v3) as v3 from veri where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen' and vayadi='$aygelen')") ;
}
...
$ilcem=$ilceninadi;
$ilceetiket='İLÇE TOPLAMI';
$toplamilce=$ilcem.' '.$ilceetiket;
$objPHPExcel->setActiveSheetIndex(0)
->setCellValue('B2', $ilinadi)
->setCellValue('B3', $ilceninadi)
->setCellValue('B4', 'İLÇE TOPLAMI')
->setCellValue('K2', $yilgelen)
...
$objPHPExcel->getActiveSheet(0)->setTitle($toplamilce);
