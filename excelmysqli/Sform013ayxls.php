...
$atarih=mysqli_query($dbh,"SELECT date_format(v175, '%d.%m.%Y')  AS yeni_tarih from veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen')") ;
while($duztarih=mysqli_fetch_array($atarih)){
$dtarih=$duztarih['yeni_tarih'];
}
$btarih=mysqli_query($dbh,"SELECT date_format(v178, '%d.%m.%Y')  AS yeni_tarih from veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen')") ;
...
$resultvyil = mysqli_query($dbh,"select ilceidi as iii,vocadi as vvv, vayadi as vayadi,
sum(v1) as v1,
sum(v2) as v2, 
sum(v3) as v3,
sum(v4) as v4, 
...
v177 as v177,
v178 as v178
from veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vocadi='$ocgelen' and vyiladi='$yilgelen' and vayadi='$aygelen') group by vocadi order by vocadi desc")  or die(mysqli_error()) ;
}else{
$resultvyil = @mysqli_query($dbh,"select  ilceidi as iii,vocadi as vvv,vayadi as vayadi,
sum(v1) as v1,
sum(v2) as v2, 
...
while($sonucum=mysqli_fetch_array($resultvyil)){
	include('sablonlar/Sform013/conf_birim_dongu.php');
...
include("sablonlar/Sform013/conf_birimasitoplam.php");
...
$objWorkSheet = $objPHPExcel->createSheet($i); //Setting index when creating
	...
	include("sablonlar/Sform013/conf_bos_birimasitoplam.php");
    $i++;
    }
$iladi=@mysqli_query($dbh,"select * from il where(ilid='$ilgelen')");
while($ilsonucum=mysqli_fetch_array($iladi)){
...
while($sonucum=mysqli_fetch_array($tresultvyil)){
	include('sablonlar/Sform013/conf_toplam_dongu.php');
	...
	if($ilcegelen=="İlçe Seçiniz"){
		...
		$tresultvyil = @mysqli_query($dbh,"select v173 as v173,v174 as v174,v176 as v176,v177 as v177 from veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vocadi='$ocgelen' and vyiladi='$yilgelen' and vayadi='$aygelen')") ;
		while($sonucum=mysqli_fetch_array($tresultvyil)){
	...
	include("sablonlar/Sform013/conf_genelasitoplam.php");
...
header('Content-Disposition: attachment;filename="Sform013_SERIK.xls"');
