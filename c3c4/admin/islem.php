...
$sorgu=mysql_db_query(b12_10384566_etf,"select * from duyuru order by yaziid desc",$baglanti);//veritabanını seçiyor ve liste genel sorgumuzu oluştuyoruz.
...
mysql_db_query(b12_10384566_etf,"delete from duyuru where yaziid='$Id'",$baglanti); //  Kaydın id değerine göre formda ajax ile gelen veriler veritabanındakiler ile değiştiriliyorr.
...
mysql_db_query(b12_10384566_etf,"Update duyuru set tarih='$tarih',konu='$konu',icerik='$icerik',dosya='$dosya',bilgi='$bilgi',resim='$resim' where yaziid='$Id'",$baglanti); //  Kaydın id değerine göre formda ajax ile gelen veriler veritabanındakiler ile değiştiriliyorr.
...
$sorgum="select yaziid,date_format(tarih, '%d.%m.%Y')  AS tarih1,konu,icerik,dosya,bilgi from duyuru order by yaziid desc";

$basla=mysql_query($sorgum);

$sonucum=mysql_fetch_array($basla); 

$cumle=$sonucum['tarih1'];
...
<td width="180" align="center" class="style5" ><? echo $cumle?></td>
