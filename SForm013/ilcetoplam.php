...
if($yilgelen==""){
  $nuf=@mysqli_query($dbh,"select distinct vyiladi,vayadi from veris where ilidi='$ilgelen' and ilceidi='$ilcegelen'");
}elseif($ilcegelen>0 and $yilgelen>0){
  $nuf=@mysqli_query($dbh,"select distinct vyiladi,vayadi from veris where ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen'");
}
...
if($yilgelen==""){
$resultvyil = @mysqli_query($dbh,"select * from veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen')") ;
}elseif($ilgelen>0 and $yilgelen>0){
  $resultvyil = @mysqli_query($dbh,"select * from veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vyiladi='$yilgelen')") ;
}
...
$ilceninadi=$ilcesonucum['ilcead'];
  ...
  $atarih=mysqli_query($dbh,"SELECT date_format(v175, '%d.%m.%Y')  AS yeni_tarih FROM veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vocadi='$ocgelen' and vyiladi='$yilgelen' and vayadi='$aygelen')") ;
  while($duztarih=mysqli_fetch_array($atarih)){
  $dtarih=$duztarih['yeni_tarih'];
  }
  $btarih=mysqli_query($dbh,"SELECT date_format(v178, '%d.%m.%Y')  AS yeni_tarih FROM veris where(ilidi='$ilgelen' and ilceidi='$ilcegelen' and vocadi='$ocgelen' and vyiladi='$yilgelen' and vayadi='$aygelen')") ;
